<?php
    ...
    if(isset($_GET['id'])){
        ...
        if(!empty($mi_id)){
            $sql = "SELECT * FROM parcial WHERE id = '$mi_id'";
            $result = $mysqli -> query($sql);
            if($result -> num_rows > 0){
                $flag = 1;
                $datos = $result -> fetch_assoc();
            }else{
                $errors[] = "No hay parciales";
            }
        }else{
            $errors[] = "ID vacio";
        }
        ...
        <div class="registro col-4 position-relative top-50 start-50 translate-middle">
            ...
            <div class="shadow-lg p-3 mb-5 bg-body rounded">
                <form action="modificaSemestre.php" method="POST">
                    <span class="">Parciales</span>
                    <input type="text" name="nombreParcial" class="form-control" aria-label="Sizing example input" aria-describedby="inputGroup-sizing-lg" value="<?php echo $datos['nombre']; ?>"><br>
